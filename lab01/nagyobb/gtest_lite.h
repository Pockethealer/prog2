...
#define CREATE_Has_fn_(X, S)                         \
    ...
    struct _Has_fn_##X##_##S                         \
    {                                                \
        template <typename C, R (C::*f)() S>         \
        struct ChT;                                  \
        template <typename D>                        \
        static char (&f(ChT<D, &D::X> *))[1];        \
        template <typename D>                        \
        static char (&f(...))[2];                    \
...
namespace gtest_lite
{
    ...
    inline std::ostream &EXPECTSTR(const char *exp, const char *act, bool (*pred)(const char *, const char *), const char *file, int line,
                                   const char *expr, const char *lhs = "elvart", const char *rhs = "aktual")
    {
        return test.expect(pred(exp, act), file, line, expr)
    ...
    inline bool eqstrcase(const char *a, const char *b)
    {
        if (a != NULL && b != NULL)
        {
